...
extension SwiftyJWT {
    ...
    static func load(_ jwt: String) throws -> (header: JOSEHeader, payload: ClaimSet, signature: Data, signatureInput: String) {
        ...
        if segments.count != 3 {
            throw SwiftyJWT.JWTErrors.InvalidToken.decodeError("Not enough segments")
        }
        ...
        guard let signature = base64decoded(signatureSegment) else {
            throw SwiftyJWT.JWTErrors.InvalidToken.decodeError("Signature is not correctly encoded as base64")
        }
    ...
    static func verifySignature(_ algorithms: [Algorithm], header: JOSEHeader, signingInput: String, signature: Data) throws {
        guard let alg = header.algorithm else {
            throw SwiftyJWT.JWTErrors.InvalidToken.decodeError("Missing Algorithm")
        }
        ...
        if verifiedAlgorithms.isEmpty {
            throw SwiftyJWT.JWTErrors.InvalidToken.invalidAlgorithm
        }
    }
}
